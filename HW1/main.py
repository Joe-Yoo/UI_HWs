...
class LoginScreen(GridLayout):
    ...
    def createPhoneInput(self):
        ...
        widget.phone.bind(focus=self.on_phone_focus_change)
    ...
    def on_phone_focus_change(self, instance, focus):
        if not focus:
            digits_only = ''.join(c for c in instance.text if c.isdigit())
    ...
    def on_phone_change(self, instance, text):
        ...
        formatted_pattern = r'^\(\d{3}\) \d{3}-\d{4}$'
        
        if re.match(formatted_pattern, text) and len(digits_only) == 10:
            self.phone_number_valid = True
            self.update_submit_button()
            return
