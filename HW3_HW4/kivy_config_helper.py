...
# Reusing from HW 2 which also just reused the example config file provided by Dr. Wilson
...
def config_kivy(window_width=None, window_height=None,
    ...
    config_window_width = Config.getint('graphics', 'width') if Config.has_option('graphics', 'width') else 800
    config_window_height = Config.getint('graphics', 'height') if Config.has_option('graphics', 'height') else 600
    ...
    if enable_simulation or simulate_device:
    ...
    return target_window_width, target_window_height, curr_device_density